...
// Implemented using documentation from Beej's guide (http://beej.us/guide/bgnet/output/html/singlepage/bgnet.html#sendrecv)
int main(int argc , char *argv[]) {
    U16 port; // -p, port of server

    for(int i = 1; i < argc; i++) { if(strcmp(argv[i], "-p") == 0) { port = atoi(argv[++i]); } } // Skip argv[0]
    fprintf(stderr, "[INFO] PORT: %d\n", port);

    // Setup listening socket for new client connection requests
    int listenfd; struct sockaddr_in server;
        ...
        if (!fork()) { // this is the child process
            ...
            while(1) {
                ...
                if ((bytesReceived = receiveMessage(connfd, buffer)) < 0) {
                    ...
                    close(connfd); free(buffer); exit(0);
                } else {
                    fprintf(stderr, "[INFO] Received message from client of size %d.\n", bytesReceived);
                    U8 operation = buffer[0]; U8 shift = buffer[1];
